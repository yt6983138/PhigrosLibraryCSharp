...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
	...
	/// and handle the code query passed to <see cref="HandleCallbackLogin(CallbackLoginData, string, bool, CancellationToken)"/>.</param>
	...
	public static async Task<TapTapTokenData> HandleCallbackLogin(CallbackLoginData loginData, string code, bool useChinaEndpoint = true, CancellationToken ct = default)
	{
	...
	/// </summary>
	/// <param name="qrCodeData">The completed QRCode data from <see cref="RequestLoginQrCode(string[], bool, CancellationToken)"/>.</param>
	/// <param name="useChinaEndpoint">Use China endpoints to login or not.</param>
	/// <param name="ct">The cancellation token to cancel the operation.</param>
	...
	public static async Task<TapTapTokenData?> CheckQRCodeResult(CompleteQRCodeData qrCodeData, bool useChinaEndpoint = true, CancellationToken ct = default)
	{
	...
	/// </summary>
	/// <param name="token">The token gotten from <see cref="CheckQRCodeResult(CompleteQRCodeData, bool, CancellationToken)"/>.</param>
	/// <param name="useChinaEndpoint">Use China endpoints to login or not.</param>
	...
	private static async Task FillHeaders(HttpRequestHeaders headers, Dictionary<string, object>? reqHeaders = null, CancellationToken ct = default)
	{
		...
		string hash = await LCHelper.GetMD5HashHexString.Invoke(timestamp.ToString(), ct);
